...
function KnittingIcon({ size = 13 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" aria-hidden="true" style={{ flexShrink: 0 }}>
      {/* needle \ */}
    ...
    <svg viewBox="0 0 200 210" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      ...
      {[0,1,2,3,4,5,6].map((row) =>
        [0,1,2,3,4,5,6,7].map((col) => {
          ...
          const cy = 26 + row * 16;
          return (
            <path key={`${row}-${col}`} d={`M${cx} ${cy+8} L${cx+6} ${cy} L${cx+12} ${cy+8}`}
              stroke="#c0b0a0" strokeWidth="1.2" strokeLinecap="round"/>
          );
        })
      )}
  ...
  return (
    <div className="pd-weight">
      {WEIGHT_ORDER.map((_, i) => (
        <div key={i} className={`pd-weight__dot${i < level ? " pd-weight__dot--on" : ""}`} />
      ))}
...
function PatternDetailPage() {
  ...
  const gaugeToolLabel = pattern.craft === "CROCHET" ? "Hook size" : "Needle size";

  /* Gauge stat callouts */
  const gaugeStats = [];
  if (pattern.gauge_stitches != null && pattern.gauge_stitches !== "")
    gaugeStats.push({ value: pattern.gauge_stitches, label: "Stitches", dot: "primary" });
  ...
  /* Shared badge rows (craft + sizes) for hero/header */
  const BadgeRows = ({ variant }) => (
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--kn-spacing-2)" }}>
      <div>
      ...
      {pattern.sizes?.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: "var(--kn-spacing-2)" }}>
          {pattern.sizes.map((size) => (
            <span key={size} className={`pd-badge pd-badge--${variant}`}>{size}</span>
          ))}
        </div>
      )}
    </div>
        ...
        <div className="pd-hero">
          <img
            ...
            onError={(e) => {
              ...
              e.currentTarget.closest(".pd-hero").nextElementSibling?.style.removeProperty("display");
            }}
          />
          <div className="pd-hero__overlay">
            <h1 className="pd-hero__title">{pattern.title}</h1>
            <BadgeRows variant="light" />
          </div>
        </div>
      ) : null}

      {/* ── Header without cover image (always rendered, hidden when hero shown) ── */}
      <div className="pd-header" style={coverImageUrl ? { display: "none" } : {}}>
            ...
            <div className="pd-gauge-stats">
              ...
              {pattern.needle_size && (
                <div className="pd-needle-row">
                  ...
                  <span style={{ color: "var(--kn-text-muted)", fontWeight: 400 }}>{gaugeToolLabel}</span>
                </div>
              )}
            </div>
          </div>
        ...
        >
          {pattern.yarns.map((yarn, i) => {
            ...
            if (yarn.meters_per_unit != null && yarn.meters_per_unit !== "")
              stats.push({ value: `${yarn.meters_per_unit} m`, label: "Per skein" });
            if (yarn.grams_per_unit != null && yarn.grams_per_unit !== "")
              stats.push({ value: `${yarn.grams_per_unit} g`, label: "Skein weight" });
            if (yarn.grams_needed != null && yarn.grams_needed !== "")
              stats.push({ value: `${yarn.grams_needed} g`, label: "Total needed" });
            if (yarn.strands > 1)
